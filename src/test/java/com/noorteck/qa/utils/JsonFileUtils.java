...
public class JsonFileUtils {
	...
	public static Map<String, Object> jsonToMap(String filePath) {
		...
		try {
			...
			JSONObject obj = new JSONObject(jsonToString);
			JSONObject scrum = (JSONObject) obj.get("scrum");

			Iterator<String> keys = scrum.keys();
			// loop through json object and store in map
			while (keys.hasNext()) {
				String key = keys.next();

				// if key is null or empty then dont add value to map
				if (scrum.get(key) != null && !scrum.optString(key).equals("")) {
					regionDataMap.put(key, scrum.get(key));
				}
			}
